...
use messages::prelude::{Actor, Address, Notifiable, Handler, Context};
use crate::signals::{SmallText, SmallNumber};
...
#[async_trait]
impl Handler<SmallNumber> for FirstActor {
    type Result = bool;
    async fn handle(&mut self, msg: SmallNumber, _: &Context<Self>) -> bool {
...
impl FirstActor {
    async fn listen_to_dart(mut self_addr: Address<Self>) {
        ...
        while let Some(signal_pack) = receiver.recv().await {
            println!("[RUST] Get message: {:?}", signal_pack.message.text);
        }
    }
}
